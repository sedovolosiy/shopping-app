...
`;
  ...
  try {
    ...
    return rawText
      ...
      // Clean the lines and assign categories based on known products
      .map(line => {
        // Remove bullet points
        const cleanedItem = line.replace(/^[\*\•\-\s]+/, '').trim();
        
        // Simple heuristic category detection
        let category = 'Uncategorized (AI Error)';
        
        // Some basic categorization by keywords
        const lowerItem = cleanedItem.toLowerCase();
        
        if (/молоко|кефир|сыр|йогурт|творог|сметан|сливк|масло сливочн|ряженка|яйца|яйцо/i.test(lowerItem)) {
          category = 'Dairy & Eggs';
        } else if (/хлеб|булк|батон|лаваш|выпечк|печен|пряник|торт/i.test(lowerItem)) {
          category = 'Bakery';
        } else if (/яблок|банан|апельсин|лимон|картошк|картофел|помидор|томат|огурц|морковь|свек|капуст|перец|лук|чеснок|укроп|петрушк|зелен|редис|салат|вешенк|шампиньон|гриб|баклажан/i.test(lowerItem)) {
          category = 'Fresh Produce';
        } else if (/мясо|говядин|свинин|курица|куриц|филе|фарш|бедрыш|голен|ветчин|колбас|сосис|бекон/i.test(lowerItem)) {
          category = 'Meat & Poultry';
        } else if (/замороженн|пельмен|мороженое|пломбир/i.test(lowerItem)) {
          category = 'Frozen Foods';
        } else if (/вода|сок|чай|кофе|напиток|газиров/i.test(lowerItem)) {
          category = 'Beverages';
        } else if (/мука|сахар|соль|рис|гречк|макарон|спагет|крупа|масло раст|подсолнечное|консерв|тунец|горошек|фасоль|кукуруз|кетчуп|майонез|приправ|оливк/i.test(lowerItem)) {
          category = 'Pantry Staples';
        } else if (/салфетк|пакет|фольга|бумаг|мыло|шампунь|посуд|средство/i.test(lowerItem)) {
